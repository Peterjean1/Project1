...
Calc::~Calc() {
	delete[] storedExpressions; // delete memory 
	storedExpressions=nullptr; // not holding value
	stored=0;// relase the memory that was in the constructor
}
...
const Calc &Calc::operator=(const Calc &rhs) {
	...
	return *this; // returning pointer to the object
}
...
double Calc::process(std::string userInput, double prevValue) {
    ...
    if(cleansedString[0]=='^'){ //power operator
